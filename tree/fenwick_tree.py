...
class FenwickTree:
    ...
    def add(self, i: int, val: int) -> None:
        ...
        if 1 <= j < len(self._data):
            self._data[j] += val
            self.add(FenwickTree._next_up(j) - 1, val)
    ...
    @staticmethod
    def _next_up(i: int) -> int:
        return i + FenwickTree._lsb(i)

    @staticmethod
    def _next_down(i: int) -> int:
        return i - FenwickTree._lsb(i)

    # 1. If we subtract 1 from i, it will be subtracted from the right most set bit and that bit will be become 0
    # 2. Then if we negate the result of step 1, the right most set bit will become 1
